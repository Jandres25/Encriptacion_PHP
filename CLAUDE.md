...
| `public/js/activity-logs-table.js`         | DataTables initialization for audit log — loaded only in `ActivityLogController::index()` via `pageScripts`; order by date DESC, pageLength 25                                          |
...
- **50 tests total:** 14 in `tests/Unit/UserTest.php` (User model), 7 in `tests/Unit/LoginAttemptTest.php` (LoginAttempt model), 10 in `tests/Unit/ActivityLogTest.php` (ActivityLog model), 19 in `tests/Integration/AuthTest.php` (Auth class)
...
| `tests/Unit/ActivityLogTest.php`  | 10 tests for `App\Model\ActivityLog`                         |
